...
def create_issue(project_key: str, summary: str, description: str, issue_type: str = "Task") -> str:
    ...
    if response.status_code == 201:
        print(f"Success! Issue created: {response.json().get('key')}")
        return f"Success! Issue created: {response.json().get('key')}"
    else:
        print(f"Failed with error {response.status_code}: {response.text}")
        return f"Failed with error {response.status_code}: {response.text}"

if __name__ == "__main__":
    # Standard MCP entry point
   create_issue("SCRUM", "Testing","tsting","Task")